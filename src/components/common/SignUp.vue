<template>
  <div class="hello">
    <h1>
      ...
      <div v-if="user.isLogin">{{this.user.userNickname}}({{this.user.userName}})님, <span v-html="user.greetingHtml"></span></div>
    ...
    <div class="container">
      <form>
        ...
        <div>
          ...
          <input type="text" id="nickname" v-model="signup.nickname" /> <button name="confirm_check_button" @click="confirm_check">중복체크 </button>
        </div>
        <br>
        <div>
        ...
        <br>
        <br>
        <button @click="goLoginPage">회원가입</button>
      </form>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    goLoginPage(e) {
      e.preventDefault();
      if (this.signup.email == "" || this.signup.password == "" || this.signup.nickname == "" || this.signup.passwordConfirm == "") {
        alert('모든 항목에 응답해주셔야 해요!')
      }
      if (this.signup.email_Id == this.user.emailid) {
        alert('이메일 1이 일치 합니다.')
      }else if(this.signup.email_Id != this.user.emailid) {
        alert('이메일과 확인이 서로 일치 하지 않습니다! 다시 확인해주세요!')
      }
      if (this.signup.email_Domain == this.user.emaildomain) {
        alert('이메일 2가 일치 합니다.')
      }else if(this.signup.email_Domain != this.user.emaildomain) {
        alert('이메일과 확인이 서로 일치 하지 않습니다! 다시 확인해주세요!')
      }
      if (this.signup.password == this.signup.passwordConfirm) {
        alert('비밀번호와 확인이 모두 일치합니다.')
      }
      else {
        return function (e){
          e.preventDefault();
